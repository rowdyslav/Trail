...
def _app_return_user_prompt(extra_context: Optional[str] = None) -> str:
    prompt = ["Событие: пользователь вернулся в приложение. Нужна короткая фраза в духе 'с возвращением' или 'продолжим'."]
...
async def generate_app_return_message(
    ...
    text = await deepseek.generate_text(
        _app_return_system_prompt(),
        _app_return_user_prompt(extra_context),
        timeout=settings.deepseek_timeout_seconds,
    )

    cleaned = " ".join(text.split())
    if not cleaned:
        raise ValueError("DeepSeek returned empty app_return reaction")
    return cleaned
